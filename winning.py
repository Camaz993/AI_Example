...
def newGeneration(old_population):
    ...
    while len(new_population) != len(old_population):
        parent_a = selectParentTournament(old_population)
        parent_b = selectParentTournament(old_population)
    ...
    #for n in range(N):
        ...
        new_creature.chromosomes = crossover(parent_a, parent_b)
    ...
    avg_fitness = np.array([get_fitness(creature) for creature in old_population]).mean()
...
def get_fitness(creature):
    fitness = (1 * creature.alive + 1* creature.turn + 1* creature.enemy_eats + 1* creature.strawb_eats + 1* creature.size)
    return fitness


def selectParentTournament(old_population):
    tournament = list()
    while len(tournament) != 5:
        tournament.append(old_population[np.random.randint(0, len(old_population))])
    best = tournament[0]
    for individual in tournament:
        if get_fitness(individual) > get_fitness(best):
            best = individual
    return best
